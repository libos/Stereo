...
uint8_t USBD_MSC_CfgFSDesc[USB_MSC_CONFIG_DESC_SIZ]  __ALIGN_END =
{
  ...
  0x0A,   /*Polling interval in milliseconds */
...
/**
  ...
  * @retval status
  */
static USBD_MSC_BOT_HandleTypeDef msbot;
...
//	uint32_t tmp = sizeof (USBD_MSC_BOT_HandleTypeDef);
//	char tc[30];
//	sprintf(tc,"sizeof USBD_MSC_BOT_HandleTypeDef:%d",tmp);
//	Debug(tc);

  pdev->pClassData = &msbot;//USBD_malloc(sizeof (USBD_MSC_BOT_HandleTypeDef));
